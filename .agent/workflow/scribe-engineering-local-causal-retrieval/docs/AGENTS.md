...
- Canonical SCRIBE CLI from a host project root is `.agent/workflow/scribe-engineering-local-causal-retrieval/scribe`; examples below use `<SCRIBE>` for that path.
