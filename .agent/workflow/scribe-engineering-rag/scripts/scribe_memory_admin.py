...
def cmd_compact(args: argparse.Namespace) -> int:
    ...
    return write_validated_scribe(store, updated, "compact")
...
def cmd_promote(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        return 0
    return write_validated_scribe(store, updated, "promote")
...
def write_validated_scribe(store: ScribeStore, updated: str, action: str) -> int:
...
def export_payload(store: ScribeStore, include_values: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "causal_edges": sum(len(v) for v in store.index.causal_edges.values()),
        },
